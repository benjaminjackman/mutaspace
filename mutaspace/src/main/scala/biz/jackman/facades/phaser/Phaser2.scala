...
// Created by bjackman @ 7/22/15 11:18 PM
//////////////////////////////////////////////////////////////
...
object Phaser {
  ...
  val VERSION: string;
  val DEV_VERSION: string;
  val GAMES: Array[Phaser.Game];
  ...
  val BITMAPDATA: number;
  val BITMAPTEXT: number;
  val BUTTON: number;
  val CANVAS_FILTER: number;
  val CIRCLE: number;
  val ELLIPSE: number;
  val EMITTER: number;
  val GRAPHICS: number;
  ...
  val POINTER: number;
  val POLYGON: number;
  val RECTANGLE: number;
  val ROUNDEDRECTANGLE: number;
  val RENDERTEXTURE: number;
  val RETROFONT: number;
  val SPRITE: number;
  val SPRITEBATCH: number;
  val TEXT: number;
  val TILEMAP: number;
  val TILEMAPLAYER: number;
  val TILESPRITE: number;
  val WEBGL_FILTER: number;
  val ROPE: number;
  ...
  val DOWN: number;
    ...
    class Animation(game: Phaser.Game, parent: Phaser.Sprite, name: string, frameData: Phaser.FrameData, frames: number[]|string[], frameRate?: number, loop?: boolean) extends js.Object {
        val currentFrame: Phaser.Frame;
        val delay: number;
        val enableUpdate: boolean;
        val frame: number;
        val frameTotal: number;
        val game: Phaser.Game;
        val isFinished: boolean;
        val isPaused: boolean;
        val isPlaying: boolean;
        val killOnComplete: boolean;
        val loop: boolean;
        val loopCount: number;
        val name: string;
        val onComplete: Phaser.Signal;
        val onLoop: Phaser.Signal;
        val onStart: Phaser.Signal;
        val onUpdate: Phaser.Signal;
        val paused: boolean;
        val speed: number;

        def complete(): void;
        def destroy(): void;
        def static generateFrameNames(prefix: string, start: number, stop: number, suffix?: string, zeroPad?: number): string[];
        def next(quantity?: number): void;
        def onPause(): void;
        def onResume(): void;
        def play(frameRate?: number, loop?: boolean, killOnComplete?: boolean): Phaser.Animation;
        def previous(quantity?: number): void;
        def restart(): void;
        def setFrame(frameId?: string|number, useLocalFrameIndex?: boolean): void;
        def stop(resetFrame?: boolean, dispatchComplete?: boolean): void;
        def update(): boolean;
        def updateFrameData(frameData: FrameData): void;
