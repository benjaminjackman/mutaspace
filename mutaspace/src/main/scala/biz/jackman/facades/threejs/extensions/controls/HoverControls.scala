...
import biz.jackman.facades.threejs.extensions.controls.HoverControls.HoverState
...
class HoverControls(camera: Camera, element: html.Element, val center: Vector3 = new Vector3()) extends RotateControls {
  ...
  def onMouseWheel(event: dom.MouseEvent) = if (enabled && userZoom) {
    ...
    if (!js.isUndefined(wheel.wheelDelta) && wheel.wheelDelta.asInstanceOf[Int] != 0) {
