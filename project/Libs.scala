...
object Libs {
  ...
  case object Cgta extends CrossLib {
    lazy val version = SettingKey[String]("cgta-version")
    override lazy val base = S(version := "0.2.2")
    override lazy val jvm = S(
      libraryDependencies += "biz.cgta" %% "oscala-jvm" % version.value,
      libraryDependencies += "biz.cgta" %% "serland-jvm" % version.value,
      libraryDependencies += "biz.cgta" %% "cenum-jvm" % version.value
    )
    override lazy val sjs = S(
      libraryDependencies += "biz.cgta" %%%! "oscala-sjs" % version.value,
      libraryDependencies += "biz.cgta" %%%! "serland-sjs" % version.value,
      libraryDependencies += "biz.cgta" %%%! "cenum-sjs" % version.value
    )
  }
